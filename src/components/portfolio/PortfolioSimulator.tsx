...
// Virtual-money trading simulator: start with ₹5,00,000, buy and sell at live
// prices, track P&L and allocation. Separate from the real-holdings tracker;
// its own localStorage key so the two never mix.
...
import { Plus, Wallet, ChartPie, X } from "lucide-react";
  ...
  function addPosition(e: React.FormEvent) {
    ...
    setState((prev) => {
      ...
      if (existing) {
        ...
        positions = prev.positions.map((p) => (p.symbol === sym ? { symbol: sym, shares: totalShares, avgPrice: avg } : p));
      } else {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.16em] font-semibold text-(--color-fg-subtle)">Practice simulator</p>
          <h1 className="mt-1 text-[24px] font-semibold tracking-tight">Practise investing — risk-free.</h1>
          <p className="mt-1 max-w-2xl text-[13.5px] text-(--color-fg-muted)">
            Start with ₹5,00,000 in virtual cash. Buy and sell at live prices. Track your P&amp;L, allocation and
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Total P&L" value={`${pl >= 0 ? "+" : "-"}₹${formatINR(Math.abs(pl))}`} delta={invested ? plPct : undefined} />
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.6fr_1fr]">
        <Card padding="md">
          ...
          <form onSubmit={addPosition} className="grid gap-3 sm:grid-cols-[1.4fr_1fr_auto]">
            ...
            <div>
              <Label htmlFor="sim-shares">Shares</Label>
              <Input id="sim-shares" type="number" min={1} value={sharesInput} onChange={(e) => setSharesInput(e.target.value)} className="mt-1.5" />
            </div>
          ...
          {error && (
            <p className="mt-3 rounded-lg border border-(--color-down)/20 bg-(--color-down-soft) px-3 py-2 text-[13px] text-(--color-down)">{error}</p>
          )}
        </Card>

        <Card padding="md">
          <CardEyebrow className="mb-3">Equity trend</CardEyebrow>
          <div className="h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={state.valueTrend} margin={{ top: 6, right: 4, left: 4, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "var(--color-surface)", color: "var(--color-fg)", border: "1px solid var(--color-border)", borderRadius: 12, fontSize: 12 }}
                  labelFormatter={(t) => new Date(t).toLocaleTimeString()}
                  formatter={(v) => [`₹${formatINR(Number(v))}`, "Equity"]}
                />
                <Area type="monotone" dataKey="v" stroke="#115e3c" strokeWidth={2} fill="url(#simFill)" isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card padding="none">
          ...
          {enrichedPositions.length === 0 ? (
            <div className="px-6 py-14 text-center">
              ...
              <p className="mt-1 text-[13px] text-(--color-fg-muted)">Add your first stock above to start trading.</p>
            </div>
          ) : (
          ...
          {allocation.length === 0 ? (
            ...
            <>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={allocation} dataKey="value" innerRadius={48} outerRadius={78} paddingAngle={2} stroke="white" strokeWidth={2}>
              ...
              <ul className="mt-2 space-y-1.5">
                {allocation.map((a) => {
                  ...
                  const p = total ? (a.value / total) * 100 : 0;
                  return (
                    <li key={a.name} className="flex items-center justify-between text-[12.5px]">
                      ...
                      <span className="font-semibold tabular text-(--color-fg)">{p.toFixed(1)}%</span>
                    </li>
                  );
                })}
              </ul>
...
function StatCard({ label, value, delta, eyebrow, accent }: { label: string; value: string; delta?: number; eyebrow?: React.ReactNode; accent?: boolean }) {
  return (
    <div className={`rounded-2xl border bg-(--color-surface) p-5 ${accent ? "border-(--color-brand-300) shadow-[0_18px_38px_-22px_rgba(13,31,23,0.16)]" : "border-(--color-border)"}`}>
