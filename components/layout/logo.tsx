...
// Ported from SOURCE stocksense's src/components/layout/Logo.tsx.
// Used only within the marketing landing page (`.stocksense-marketing`).
...
export function Logo({
  ...
  const fg = tone === "dark" ? "text-white" : "text-[var(--color-fg)]";
  return (
    <Link
      href={href}
    ...
    >
      ...
      <span className="text-[17px]">
        Stock<span className={tone === "dark" ? "text-[var(--color-brand-300)]" : "text-[var(--color-brand-600)]"}>Sense</span>
      </span>
    </Link>
  );
}
