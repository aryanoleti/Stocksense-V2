...
export function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[var(--color-brand-950)] text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-5 pt-24 pb-40 sm:pb-48">
        <div className="grid gap-14 md:grid-cols-[1.3fr_1fr_1fr_1fr_1fr] md:gap-10">
          ...
          {COLUMNS.map((c) => (
            <div key={c.title}>
              <p className="text-[10.5px] uppercase tracking-[0.2em] font-semibold text-[var(--color-brand-300)]">
                {c.title}
              </p>
                  ...
                  <li key={l.label}>
                    <Link href={l.href} className="group inline-flex items-center gap-1 text-[14.5px] text-white/70 hover:text-white">
                      <span className="relative">
                        {l.label}
                        <span className="absolute -bottom-0.5 left-0 h-px w-0 bg-[var(--color-brand-300)] transition-[width] duration-300 group-hover:w-full" />
                      </span>
                      <ArrowUpRight className="h-3 w-3 -translate-x-1 text-[var(--color-brand-300)] opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100" />
                    </Link>
                  </li>
                ))}
              </ul>
