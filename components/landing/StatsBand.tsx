...
const STATS = [
  ...
  { to: 30, format: (v: number) => `${Math.round(v)}s`, label: "Quote refresh cycle (10–30s)" },
];

export function StatsBand() {
  const { ref, shown } = useReveal<HTMLDivElement>();
  return (
    <section id="stats" className="border-y border-[var(--color-border)] bg-[var(--color-surface)]">
      <div ref={ref} className="mx-auto grid max-w-7xl grid-cols-2 gap-y-12 px-5 py-20 md:grid-cols-4">
        {STATS.map((s) => (
          <div key={s.label} className="text-center">
            <p className="text-[40px] font-semibold tabular tracking-[-0.03em] text-[var(--color-fg)] sm:text-[48px]">
              <Counter to={s.to} format={s.format} start={shown} />
            </p>
            <p className="mt-1.5 text-[13px] text-[var(--color-fg-muted)]">{s.label}</p>
          </div>
        ))}
      </div>
    </section>
