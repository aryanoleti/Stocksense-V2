...
const FAQS = [
  ...
  },
  {
    ...
    a: "Individual stock quotes refresh roughly every 10 seconds, and batch views (like watchlists) every 30 seconds, with a small cosmetic jitter in between so the number keeps moving — all reflecting cached NSE data with a short delay. For execution decisions on real money, always confirm with your broker.",
  },
  {
...
export function FAQ() {
  return (
    <section id="faq" className="border-t border-[var(--color-border)] bg-[var(--color-surface-2)]/50">
      <div className="mx-auto max-w-3xl px-5 py-24">
        <div className="text-center">
          <p className="text-[11.5px] font-semibold uppercase tracking-[0.18em] text-[var(--color-brand-700)]">
            Questions, answered
          </p>
        ...
        <ul className="mt-12 divide-y divide-[var(--color-border)] rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]">
          {FAQS.map((f, i) => (
  ...
  return (
    <li>
      <button
        ...
        aria-expanded={open}
      >
        <span className="text-[15.5px] font-semibold tracking-tight text-[var(--color-fg)]">{q}</span>
        <ChevronDown
          className={cn(
      ...
      >
        <div className="min-h-0">
          <p className="text-[14.5px] leading-relaxed text-[var(--color-fg-muted)]">{a}</p>
        </div>
      </div>
    </li>
  );
