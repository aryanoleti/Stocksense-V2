...
export function FeatureCinema() {
  ...
  return (
    <section id="features" className="relative bg-[var(--color-bg)]">
      ...
      <div className="absolute inset-x-0 -top-px h-24 bg-gradient-to-b from-[var(--color-brand-950)] to-transparent" />
        ...
        <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
          ...
          <div className="hidden lg:block">
            <div className="sticky top-28">
              <p className="text-[11px] uppercase tracking-[0.22em] font-semibold text-[var(--color-brand-600)]">
                Why StockSense
              </p>
              <h2 className="mt-3 text-4xl font-semibold tracking-[-0.03em] text-[var(--color-fg)] sm:text-[44px] sm:leading-[1.06]">
              ...
              <div className="mt-10 space-y-1">
                {FEATURES.map((f, i) => (
                  ...
                  >
                    <f.icon
                      className={cn("h-[18px] w-[18px]", active === i ? "text-[var(--color-brand-700)]" : "text-[var(--color-fg-subtle)]")}
                    />
                    <span className="text-[14.5px] font-semibold tracking-tight text-[var(--color-fg)]">
                      {f.title}
                    </span>
                  </div>
                ))}
          ...
          <div className="space-y-24 lg:space-y-40">
            <div className="lg:hidden">
              ...
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[var(--color-fg)]">
  ...
  return (
    <div ref={centerRef}>
      <div ref={ref} className={cn("reveal", shown && "reveal-shown")}>
        <p className="text-[11.5px] uppercase tracking-[0.18em] font-semibold text-[var(--color-fg-subtle)]">
          {feature.eyebrow}
        </p>
        <h3 className="mt-2.5 text-[26px] font-semibold tracking-[-0.02em] text-[var(--color-fg)] sm:text-[30px]">
          {feature.title}
        </h3>
        <p className="mt-3 max-w-lg text-[15px] leading-relaxed text-[var(--color-fg-muted)]">{feature.body}</p>
        <div className="mt-8">
          <Visual />
        </div>
      </div>
...
function Frame({ children, shown, innerRef }: { children: React.ReactNode; shown?: boolean; innerRef?: React.Ref<HTMLDivElement> }) {
  return (
    ...
    >
      <SpotlightCard className="overflow-hidden rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-lg)]">
        {children}
      </SpotlightCard>
    </div>
  );
    ...
    <Frame innerRef={ref} shown={shown}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11.5px] text-[var(--color-fg-subtle)]">Reliance Industries</p>
        </div>
        <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-up-soft)] px-2 py-0.5 text-[11px] font-semibold text-[var(--color-up)]">
      ...
      <div className="mt-5 flex h-28 items-end gap-1.5">
        {bars.map((h, i) => (
          <div
            key={i}
            className={cn("grow-transition flex-1 rounded-t-md", i % 3 === 0 ? "bg-[var(--color-brand-200)]" : "bg-[var(--color-brand-600)]")}
            style={{ height: shown ? `${h}%` : "0%", "--grow-delay": `${i * 50}ms` } as React.CSSProperties}
          />
        ))}
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-[var(--color-border)] pt-3 text-[11.5px] text-[var(--color-fg-subtle)]">
  ...
  return (
    <Frame innerRef={ref} shown={shown}>
      <div className="flex justify-end">
        <div className="rounded-2xl rounded-tr-md bg-[var(--color-brand-700)] px-4 py-2.5 text-[13px] text-white">
          Compare HDFC Bank and ICICI Bank
        </div>
      </div>
      <div className="mt-3 flex gap-2.5">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[var(--color-brand-50)] text-[var(--color-brand-700)]">
          <Sparkles className="h-3.5 w-3.5" />
        </span>
        <div className="space-y-2">
          <div
            className={cn("reveal rounded-2xl rounded-tl-md border border-[var(--color-border)] bg-[var(--color-surface-2)]/70 px-4 py-2.5 text-[13px] leading-relaxed text-[var(--color-fg)]", shown && "reveal-shown")}
          ...
          <div
            className={cn("reveal flex items-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2", shown && "reveal-shown")}
            style={{ "--reveal-delay": "480ms" } as React.CSSProperties}
          >
            <span className="text-[10px] uppercase tracking-[0.12em] font-semibold text-[var(--color-fg-subtle)]">Confidence</span>
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--color-surface-2)]">
              <div
                className="grow-transition h-full rounded-full bg-gradient-to-r from-[var(--color-brand-300)] to-[var(--color-brand-600)]"
  ...
  return (
    <Frame innerRef={ref} shown={shown}>
      <div className="flex items-baseline justify-between">
        <p className="text-[13px] font-semibold text-[var(--color-fg-subtle)]">Virtual portfolio</p>
        ...
        {rows.map((r, i) => (
          <div key={r.sym} className="flex items-center gap-3">
            ...
            <div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--color-surface-2)]">
              <div
                className={cn("grow-transition h-full rounded-full", r.up ? "bg-[var(--color-brand-500)]" : "bg-[var(--color-down)]/70")}
                style={{ width: shown ? r.w : "0%", "--grow-delay": `${300 + i * 140}ms` } as React.CSSProperties}
              />
            </div>
            <span className={cn("w-16 text-right text-[12px] font-semibold tabular", r.up ? "text-[var(--color-up)]" : "text-[var(--color-down)]")}>
              {r.pnl}
            </span>
          </div>
        ))}
    ...
    <Frame innerRef={ref} shown={shown}>
      <div className="space-y-3">
        {items.map((n, i) => (
          <div
            key={n.t}
            className={cn("reveal rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-2)]/60 p-4", shown && "reveal-shown")}
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={cn(
                  ...
                  n.up ? "bg-[var(--color-up-soft)] text-[var(--color-up)]" : "bg-[var(--color-down-soft)] text-[var(--color-down)]",
                )}
              >
                {n.s}
              </span>
            </div>
            <p className="mt-2 flex items-center gap-1.5 text-[12px] text-[var(--color-brand-700)]">
