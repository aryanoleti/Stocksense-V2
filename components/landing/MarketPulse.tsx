    ...
    <section ref={sectionRef} className="relative" style={{ height: reduced ? "auto" : "340vh" }}>
      ...
      >
        <div ref={headerRef} className="mx-auto w-full max-w-7xl px-5" style={{ willChange: "opacity" }}>
          <p className="text-[11px] uppercase tracking-[0.22em] font-semibold text-[var(--color-brand-300)]">
            Live screens
          </p>
        ...
        {!reduced && (
          <div className="mx-auto mt-12 w-full max-w-7xl px-5">
            <div className="h-px w-full bg-white/10">
              <div ref={lineRef} className="h-px origin-left bg-[var(--color-brand-300)]" style={{ transform: "scaleX(0)" }} />
            </div>
          </div>
        )}
      </div>
...
function EndCard() {
  return (
    <Link href="/stocks" className="group block shrink-0">
      <div className="flex h-full w-[min(320px,75vw)] flex-col items-start justify-center rounded-3xl border border-dashed border-white/15 p-8 transition-colors group-hover:border-[var(--color-brand-300)]/60">
        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[var(--color-brand-300)]/15 text-[var(--color-brand-300)]">
        ...
        <span className="mt-5 inline-flex items-center gap-1.5 text-[13px] font-semibold text-[var(--color-brand-300)]">
